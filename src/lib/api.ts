...
export async function fetchAllPhotos(): Promise<ApiPhoto[]> {
  ...
  if (!firstPage.ok) throw new Error(`Failed to fetch photos: ${firstPage.status}`);
...
export async function fetchPhotosByCategory(slug: string): Promise<ApiPhoto[]> {
  const firstPage = await fetch(`${API_BASE}/photos?category=${slug}&page=1&size=100`);
  if (!firstPage.ok)
    throw new Error(`Failed to fetch photos for category "${slug}": ${firstPage.status}`);
  const data: { data: PaginatedPhotos } = await firstPage.json();
  const { items, pages } = data.data;
